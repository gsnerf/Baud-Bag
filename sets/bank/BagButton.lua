...
function BaudBag_Bank_BagButtonMixin:UpdateContent()
    -- ensure we load potentially cached data when opening the bank in offline mode before visiting the bank npc
    if not self.TabData and not AddOnTable.State.BankOpen then
...
function BaudBag_Bank_BagButtonMixin:OnShow()
    self:UpdateContent()
end
...
function BaudBag_Bank_BagButtonMixin:OnClick(button)
    if button == "LeftButton" and self.TabData then
        AddOnTable.Functions.DebugMessage("Bank", "BagButton#OnClick: recognized left click on already bought bank tab", self.TabData, self.SubContainerId)
    ...
    if button == "RightButton" and self.TabData then
        AddOnTable.Functions.DebugMessage("Bank", "BagButton#OnClick: recognized right click on already bought bank tab", self.TabData, self.SubContainerId)
...
function BaudBag_Bank_BagButtonMixin:OnCustomLeave()
    local tooltip = GameTooltip --BaudBagBagsFrameTooltip
    tooltip:Hide()
